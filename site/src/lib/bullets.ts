/** Read the bullet catalogue from data/bullets/<key>.json and its generated search index. */
...
/** The whole catalogue record, fetched when a bullet is opened. */
...
export function loadBullet(key: string): Promise<BulletRecord> {
  ...
  const request = fetch(`${import.meta.env.BASE_URL}data/bullets/${key}.json`).then((response) => {
 ...
 *
 * Catalogue dimensions can differ slightly from a bullet's nominal diameter.
 * This tolerance supports catalogue matching; it does not establish compatibility.
 */
