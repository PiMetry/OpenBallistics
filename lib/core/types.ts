 ...
 *
 * Framework-free: these are the dataset's own shapes and must stay importable without Svelte.
 * Anything needing the reader's language - `familyLabel`, for one - belongs in the app layer,
 * not here (see `site/src/lib/types.ts`).
 */
 ...
 */
type Confidence = 'verified' | 'unverified' | 'implausible';
 ...
 *
 * `file` names the drawing - the path the renderer files it under, which is how the manifest
 * lists it and how a page tells one drawing of a cartridge from another. Nothing is fetched by
 * it: the page draws the picture itself, in whichever of four faces it wants (2026-09-05) - the
 * coloured rendering or the dimensioned outline, each with or without the dimensions. So a
 * drawing has no style of its own any more; the page has.
 *
...
/** The case families the dataset publishes, in the order the site lists them. */
export const FAMILIES: string[] = ['rimless', 'rimmed', 'belted', 'pistol', 'rimfire', 'shotshell'];
